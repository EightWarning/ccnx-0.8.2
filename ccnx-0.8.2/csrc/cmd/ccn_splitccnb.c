...
segment_prefix(char *path)
{
    ...
    s = strrchr(path, '/');//strrchr：查找一个字符c在另一个字符串str中末次出现的位置, 返回c及以后字符串，否则null
    if (s == NULL) s = path;
    d = strrchr(s, '.');
    if (d == NULL) d = s + strlen(s);
    r = calloc(1, 1 + d - path);//在内存的动态存储区中分配n个长度为size的连续空间，函数返回一个指向分配起始地址的指针
    memcpy(r, path, d - path);//void *memcpy(void *dest, const void *src, size_t n)，从源src所指的内存地址的起始位置开始拷贝n个字节到目标dest所指的内存地址的起始位置中
    return (r);//函数返回指向dest的指针
}
